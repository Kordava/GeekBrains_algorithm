...
import numpy as np
# ------------------------------------------------------------------------------------------------
# 1. Четвертый шаг улучшения - массивы numpy.
...
# 2. Результаты.
    ...
    # Итог: работает медленне, но не критично. Памяти в 4 раза меньше.

    # Общий итог: Использование памяти снижено в 1200 раз по сравнению с начальным вариантом,
    # удвлось устранить ключевой недостаток алгоритма "решета" без потери функции скорости
    # выполнения (скорость отличается на константу)
...
def ert(iter1):
    ...
    start = 0
    stop = num
    my_list = np.array(list(range(start, stop)))
    ...
    while True:
        ...
        if i > 1:
            ...
            if i == num:
                ...
                prime_numbers = np.array(list(set(my_list)))
                prime_numbers = np.delete(prime_numbers, 0)
